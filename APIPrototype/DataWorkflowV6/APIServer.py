...
'''Program that starts the API server and defines routes processed by it.
   Conducts queries to a PostgreSQL database using DBQuery.py as a helper
   script, table to be queried defined by user in route address (<table> variable)

   Current server address: http://127.0.0.1
'''
...
def getRecord(table, uuid):
    ...
    #Get record from DB using variables from route
